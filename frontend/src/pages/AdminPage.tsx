...
export function AdminPage() {
  ...
  async function handleTrigger() {
    ...
    setError(null)
    try {
      const run = await endpoints.triggerIngestion(apiFetch)
      setLastRun(run)
    } catch {
  ...
  return (
    <Layout>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-border bg-surface p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              <h2 className="text-sm font-semibold text-ink">Resumen de la corrida</h2>
      ...
      >
        {triggering ? 'Ejecutando ingesta…' : 'Disparar ingesta ahora'}
      </button>
    </Layout>
  )
}
